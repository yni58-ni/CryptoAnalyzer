...
/**
 * This class represent line chart viewer
 * @author gracezhu
 *
 */
public class LineChartViewer extends Viewer{
	...
	/**
	 * Constructor to initialize line chart viewer
	 * @param sel user selection
	 * @res result 
	 */
	protected LineChartViewer(Selection sel, Result res) {
		super(sel, res);
	...
	protected void generateChart() {
		...
		for(int i=1; i<row; i++) {
			String s = result[i][0]+" - "+select.getFreq();
