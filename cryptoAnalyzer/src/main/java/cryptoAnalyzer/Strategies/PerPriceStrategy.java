...
/**
 * This class represent calculation for percentage change of price
 * @author gracezhu
 *
 */

public class PerPriceStrategy extends Strategy {
	
	/**
	 * perform calculation for percentage change of price
		...
		for(int i=0; i<res.getRow(); i++) {
			...
			for(int j=0; j<res.getCol(); j++) {
				if(i==0) {
					...
					if(j==0) {
						res.setCryptoName(cryptoList[i-1], i); //first column id list of crypto
					}else {
